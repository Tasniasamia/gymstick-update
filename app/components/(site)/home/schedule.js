        ...
        <div className="flex md:gap-[60px] gap-[20px] flex-wrap mx-auto md:justify-center mb-[40px]">
          <div
            className={`px-[30px] py-[12px] cursor-pointer duration-300 transition-all capitalize ${
              category === "all" ? "bg-primary text-white" : "bg-white text-black"
            }`} 
          ...
          {ScheduleData.schedule.map((schedule, index) => (
            <div
              key={index}
              className={`px-[30px] py-[12px] cursor-pointer duration-300 transition-all capitalize ${
                category === schedule?.category ? "bg-primary text-white" : "bg-white text-black"
              }`} 
          ...
          <tbody>
            {(category === "all" ?allWorkouts:filterbyCategory)?.map((workout, index) => (
              <tr key={index} className="border">
                <td className="w-[180px] h-[80px] border-r bg-primary text-white">
                  {workout?.time}
                </td>
                <td className="w-[180px] h-[80px] border-r hover:bg-primary hover:text-white duration-300 transition-all  capitalize cursor-pointer">
                  {workout?.Monday}
                </td>
                <td className="w-[180px] h-[80px] border-r hover:bg-primary hover:text-white duration-300 transition-all  capitalize cursor-pointer">
                  {workout?.Tuesday}
